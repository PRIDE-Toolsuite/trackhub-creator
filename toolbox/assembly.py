...
class AssemblyMappingServiceFromStaticFile(AssemblyMappingService):
    ...
    def _get_raw_assembly_data_object(self):
        if not self.__raw_assembly_data_object:
            ...
            # self._logger.info("Assembly Mapping Data ---> '{}'".format(response.content.decode()))
            self.__raw_assembly_data_object = json.loads(response.content.decode())
    ...
    def _get_index_by_ensembl_accession_mapping_entry(self):
    ...
    def get_ucsc_assembly_for_ensembl_assembly_accession(self, ensembl_assembly_accession):
        if not ensembl_assembly_accession in self._get_index_by_ensembl_accession_mapping_entry():
        ...
        mapping_entry = self._get_index_by_ensembl_accession_mapping_entry()[ensembl_assembly_accession]
